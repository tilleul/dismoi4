...
catch (Exception) {console.log("     pas de tag out      ");}
...
//console.log('nom de plugs'+nomplugin) ;
 callback({'tts': "il y a " + nombreplugin + ",pluguine"}) ; SARAH.speak("qui se nomme " + nomplugin);
console.log('le json seras alors '+data1)
...
//console.log("objet1111111111111111111111111111" + cd1);
  ...
  pathname = 'C:/Users/Administrateur/sarah/plugins/'+cd1+'/'+cd1+'.xml';
...
var fs = require('fs');
fs.writeFile("C:/Users/Administrateur/sarah/plugins/dismoi/dismoi1.xml", datas_xml, function(err) {
      if(err) {console.log(err);}
      else {console.log("plugin courses généré!");}})
...
catch (Exception) {console.log("     pas de tag out      ");SARAH.speak('pas de phrase dans le xml')}
...
catch (Exception) {console.log("     pad de tag out      ");}
...
// on lis le xml  on parse tout cela
    parser.parseString(data, function (err, result) {if (err) {console.log('eeeeeeeeeeeeeeeeeerreur de xml')};
        // on récupere la ligne item complete
        //  item1 = result.grammar.rule[0]['one-of'][0].item ;
         item1 = result.grammar.rule[0].item ;

  nombreitem1 = item1.length;console.log('nombre de oneof'+nombreitem1)
...
console.log('le json seras alorssssssssssssssssssssss '+data1)
 pathname = 'C:/Users/Administrateur/sarah/plugins/'+cd1+'/'+cd1+'dismoi.json';
console.log('nom du plug'+pathname);
